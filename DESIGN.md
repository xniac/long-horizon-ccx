...
方向 **Long-horizon**，目标 agent **Claude Code**。交付两部分：(1) 给 Claude Code 补齐长程任务执行能力的扩展模块 `lhx`；(2) 回答「这次改动在产品里真的更好吗」的评估系统。全文按 **Agent / Harness /
...
| **Harness**（§2） | `lhx` 六个 hooks + 磁盘状态 + evaluator | 包在模型外、补长程能力的模块，**被测对象** |
| **Task**（§3） | `lhxeval/tasks/synthetic/*.json`（prompt/setup/verify/参考解） | 任务规格，含 golden-behavior 回归子集 |
| **Eval**（§4） | runner + sandbox + graders + metrics + stats + dashboard | 打分、量化不确定性、呈现结果 |

> 注：本文的 Harness 取 Anthropic *Effective harnesses* 的含义，即 `lhx`。Harbor /
...
| 只跑真实后端时，评估非确定、要计费，且没有 ground truth，无法先校准装置、再相信它量出来的数字 | 加一个确定性的 `simulated` backend，与 `claude -p` CLI、Python Agent SDK 共用同一套 runner/grader/指标（真实跑用 `--output-format json` 抓 cost/token） | 既能离线、零成本、确定地校准装置（§2.3），又能在同一份任务/指标上跑真实模型看能力 |
...
每个缓解都可独立开关，支持逐个消融。参考来源：Anthropic *Effective harnesses* /
...
| 局限（现状） | 改进 | impact |
|---|---|---|
| cwc 的 `verify-gate.sh` 靠「读过某个后缀的文件」判定完成，这只是证据代理、可以被绕过 | 换成 completion gate，由 fresh-context evaluator 复现成功标准来判定 | 完成评分不再依赖 agent 自评；代价是 cwc 的硬性写入门槛没有移植（列入 §6） |
| cwc 没有评估，也没有 long-horizon 指标 | 配套整套配对 A/B + 长程专属指标 | §4、§5 |

**门控的盲区**：completion gate 判据是 `feature_list.json` 全部 pass，而这个字段由 agent 自己
`mark_pass` 翻转，所以门控本身可以被自报绕过，只有离线评分的 `verify` 能兜住。在产品语境下（线上没有 eval verify 兜底时）这就是真实的 reward-hacking 空子，改进方向见 §6（让门控也参考 verify 结果）。

### 2.3 Harness 验证：已知效应下的仿真校准

**局限**：真实跑是非确定的、没有 ground truth，无法判断「模块 + 度量装置」本身是否正确。

**改进**：仿真 backend 用显式概率建模 M2–M7，每个缓解都**挂在真实的 `Config` 开关上**；配合参考解
sanity check（参考解必须 pass、空产出必须 fail）、regression 阴性对照，以及 54 项单测（pass@k
...
配对 Δ +0.406 [+0.331, +0.481]；McNemar helped=66、hurt=1、p≈9e-19。regression 子集是干净的阴性对照（两臂都 1.000，无误报），唯一的 hurt 是一个反方向的 discordant pair，属噪声。

**这个验证的边界**：失效概率只施加在 OFF 臂上，仿真在构造上只可能得出 ON 更好，所以它**不**证明模块的真实能力。它连同 54 项单测，证明的是**评估管道的数值正确性**（统计公式与跨进程确定由单测保证，端到端接线、聚合、regression 对照无误报由这次跑通验证），但**没有**验证两件评估里最难的事：
grader 能否抵御 reward hacking、指标在真实噪声下的区分度，这两件只能靠 §5 的真实 A/B。

### 2.4 验证过程抓出的两个真实 bug

这两个 bug 只看汇总 pass/fail 发现不了，查事件轨迹才暴露；两个都补了回归测试。
...
评估要回答两个产品问题：一次改动（模型发布时是新版本，本方案里是模块 ON/OFF）**让它更强了吗**，以及**有没有破坏原有行为（golden behavior）**。两类任务分别管这两问：
...
**局限**：现在的 agent 很强、会自动调工具，多数任务一个 session 就做对了，ON 反而只多付通信开销（§5 的 v01–v03 行）。OFF 也能成功，是因为总有逃生口：pytest 给出确切的 N/M 通过数、文件系统暴露历史决策、任务塞得进单会话、或 shell 批处理绕过 context。这些逃生口来自 v03/v04 与一系列未收录的探索任务（cascading-bug、config-driven、orphan-file 等变体）的实跑教训。
...
| | model-judge（未实现，列入 §6） | 主观/开放式产出 | 需校准、自身有方差 |
...
doom-loop 频次）；**token/资源效率也是一等公民**（v07 唯一的收益就是效率，§5.3）。
...
+1.00]` 是 bootstrap 在「三点全同」上的退化产物、无信息量，以 McNemar 的 p 下限为准（§5.1）。
...
每个威胁对应一个已实现的应对，落点给出实现所在章节：

| 威胁 | 应对 | 落点 |
|---|---|---|
| 任务歧义（完成标准说不清） | `verify` 让完成标准机器可判；参考解证明任务可解 | §3.2、§2.3 |
| grader 被自报绕过 | 评分只认可执行检查与磁盘产物 | §2.2、§4.2 |
| 环境不稳 / 跨 trial 污染 | 每 trial 全新沙箱、跑完销毁 | §4.5 |
| 模型非确定 | 配对同种子 + 每任务 k 次重复 | §4.1 |
| 任务饱和（两臂都满分，测不出差异） | 单会话任务明确标为负对照；正 delta 任务按必要条件构造 | §3.2、§5 |
| 度量装置自身算错 | 已知效应仿真校准；事件轨迹核查（抓出两个真实 bug） | §2.3、§2.4 |

任务与 grader 自身的缺陷足以废掉整个基准：SWE-bench Verified 在 2026-02 就因测试缺陷 + 训练污染[被 OpenAI 停用](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/)。每个任务必须带可通过 verify 的参考解（§2.3），防的正是这类事故。

---

## 5. 真实 A/B：逐用例结果（Claude Haiku 4.5，executable verify）

| 用例 | 结论 | 机制 | 有正收益？ |
|---|---|---|---|
| **v01–v03** 单会话量级 | ON=OFF 成功，ON 多付 ~30–50% token | `claude -p` 不强制 compaction，跨会话机制没被触发 | 否（正确的负对照） |
| **v04** 178k-token audit | ON=OFF，无 compaction，每 trial $0.079、11 次工具调用 | Haiku 用 `sed`/`python3 -c` 批处理，178k token 从没进 context | 否（长程场景被绕过，保留作负例） |
| **v05** build 跨会话 | ON 3/3 vs OFF 0/3 | 见 §5.1 | **是（success）** |
| **v06** session-scoped debug | ON 3/3 vs OFF 0/3 | 见 §5.2 | **是（success，第二种形态复现）** |
| **v07** amnesia + pytest 限制 | 两臂都成功，ON −45% token | 见 §5.3 | **是（efficiency）** |

三个正收益用例的原始数据（逐 trial `results.json` + dashboard）都提交在
[`runs_archive/`](runs_archive/)，数字可直接核对、不必重跑计费。

### 5.1 v05（k=3，max_turns=80，max_sessions=3，$2.16，`runs_archive/v05_k3/`）

任务：跨 session 构建 10 个相互 import 的模块，prompt 限定一个 session 只建一个模块、动工前先读
PROGRESS.md；build 场景没有现成测试，「全部建完了吗」只能 agent 自判。

| metric | ON | OFF | paired Δ |
|---|---|---|---|
| success | **3/3** | **0/3** | +1.000（见下方统计说明） |
| partial | 1.000 | 0.308 | +0.692 |
| tokens/trial | 29,392 | 5,650 | +23,742 |
| cost/trial | $0.599 | $0.121 | ~5× |

**机制**：**ON** 在 session 1 建好约 9 个模块、逐个 `mark_pass`，Stop 门控见还没全部 pass 就拦下来、迫使 agent 继续；session 2 补完最后一个模块和测试，verify 通过。**OFF** 每个 session 只做 2–4 个模块、自报「做完了」、没有门控拦截就早停，3 个 session 后仍卡在 partial 0.31。三个种子零方差。tradeoff：ON 贵 5 倍但 100% 成功，OFF 便宜但 100% 失败。

**Caveat**：prompt 要求「一 session 只建一个模块」而 max_sessions(3) 小于模块数(10)，遵守指令的 OFF 在算术上就不可能完成；ON 赢在门控迫使它突破这条指令、在一个 session 里连做多个模块。所以这个 delta 带构造成分：它是门控机制的受控演示，不是真实使用中自然出现的差距（§6 列了去构造化的跑法）。

> 统计说明：n=3 下 success delta 的 bootstrap CI 退化为 `[+1.00, +1.00]`，不当真置信区间用；
> McNemar 在 n=3 discordant 时 p 触底于 0.25，是样本量下限，不是效应量问题（§6 会把 k 提到 ≥5）。

### 5.2 v06（k=3，max_turns=60，max_sessions=4，$2.03，`runs_archive/v06_k3/`）

任务：计算器库拆成 8 个小模块、各埋 1 个 bug、各配独立测试文件；prompt 限定一个 session 只修一个
模块、只准读和跑该模块及其测试、禁跑全量套件。

| metric | ON | OFF | Δ |
|---|---|---|---|
| success | **3/3** | **0/3** | +1.000（同 §5.1 统计说明） |
| partial | 1.000 | 0.500 | +0.500 |
| tokens/trial | ~21,552 | ~11,809 | +9,742 |

**机制**：与 v05 同型，全局测试套件被按 session 隐藏后，「整个库修完了吗」退化成主观判断，正是 M2。门控迫使 ON 在 1–2 个 forced session 里修完全部 8 个模块；OFF 遵守「一模块一 session」、又没有全局 oracle 提示，几个 session 后停在约 4/8。

**Caveat（同 §5.1）**：max_sessions(4) 小于模块数(8)，OFF 会先耗尽会话预算；这两个用例合起来证明的是门控机制在 build 和 debug 两种形态上各自复现。

### 5.3 v07：方向相反的收益（k=2，`runs_archive/v07_k2/`）

任务：3 个文件里埋 10 个 subtle bug，全局套件（15 个测试）是唯一 oracle；prompt 强调跨会话失忆
（只有 PROGRESS.md 和 feature_list.json 存续）、每 session 最多跑两次 pytest。

ON 不总是更贵。这里两臂都成功、没有 success delta，但 ON 的 PROGRESS.md 让每个冷启动 session 省掉「重新搞清楚哪些已经修好」的重复开销：**ON −45% token / −12,652 [−14,380, −10,924]，统计显著**。需要「逼着做完更多」时 ON 更贵（v05/v06），需要「跨会话重新定向」时 ON 反而更省（v07）。

### 5.4 评估过程中抓出的两个 `lhx` bug（拿到 v05 正 delta 之前）
...
## 6. 未来工作
- **去掉 v05/v06 的构造成分**：把 max_sessions 放宽到接近模块数，让 OFF 的失败更多来自真实的「过早宣布完成」而非会话预算耗尽。
- **真实触发 M3（context rot）**：构造一个特性多、且内联大体量上下文（~168k token）的任务，逼真实 Claude 走到 compaction，检验状态落盘 + `MEMORY.md` 在真实压缩下的存活；v04 的教训是 agent 会用 shell 批处理绕开，需配合 §3.2 的工具白名单。
- **M5/M6 的真实测量**：doom-loop 频次与目标漂移目前只在仿真里验证过。真实 A/B 的事件轨迹由模块自身的 hook 记录，OFF 臂 hook 关闭、轨迹为空，两臂无法对称计数；漂移检查也只对没有 verify 的任务启用。需要独立于模块的轨迹采集（解析 transcript，或一个两臂都挂载的仅记录 hook），这两个指标才能进真实 A/B。
...
- **Harbor adapter** 接 Terminal-Bench 2.0 / SWE-bench-Pro：`verify` 与 Harbor 的 pytest-F2P、OSWorld 的 evaluator 脚本同构（都按产出的真实状态判定），所以是 adapter 工作量而非重构；本 take-home 因算力有限未实跑，但离线仿真 + 小规模真实 A/B 已验证链路每环。
...
- **收紧统计与门控**：k 提到 ≥5，把 McNemar 从 p=0.25 下限推到 <0.05；让完成门控也参考 verify
  结果（堵住 §2.2 的自报空子）；引入 model-judge 处理开放式产出；给 trial 执行加 Docker 隔离；
  在 step-budget 之外加 `max_budget_usd` 成本熔断。

## 7. 安全 & 复现
最小权限 `allowedTools`（evaluator 不给 Write/Edit）、kill-switch + `STEER.md` 提供人工介入、
step-budget 熔断、隔离沙箱、完整 transcript 审计。

沙箱边界的现状：trial 目录用 `mkdtemp` 建在系统临时目录（0700，仅本用户可读），防的是
trial 间泄漏与本机其他用户；但真实 backend 以 `bypassPermissions` 跑在宿主机上，agent 进程本身
**不受**沙箱目录约束（它能写本用户能写的任何路径）。按本仓库的任务集这是可控的取舍；
跑不可信任务规格前应先落 §6 的 Docker 隔离。

```bash
pip install -e ".[dev]" && python scripts/seed_tasks.py && lhx-eval validate
pytest -q                                            # 54 tests（pytest 由 dev extra 提供）
lhx-eval run -k 10                                   # 仿真校准（16 任务）→ runs/latest/dashboard.html

# 真实 Claude：.env 里配 ANTHROPIC_API_KEY；transport 自动选 PATH 上的 claude CLI，
# 没装 CLI 则需 pip install -e ".[sdk]" 改走 Python SDK。每个任务的多会话设置随任务
# 定义走（schema.RunConfig 自动生效），直接 --task-id 即可复现，不必记 env 旋钮：
cp .env.template .env
