...
| 跨会话协调 | v05（10 模块 build，k=3） | ON 3/3 success vs OFF 0/3，Δ +1.00 [+1.00, +1.00]；5× cost 换可靠性 |
...
### 5.3 任务集
当前 14 个合成任务（[scripts/seed_tasks.py](scripts/seed_tasks.py) 生成并校验）：**capability**
...
`v04-bigrepo-audit`（capability，含 `setup` 种入 50 模块的大仓库以施加 context 压力）、
**`v05-incremental-app`（capability，10 个相互 `import` 的模块 + prompt 显式"一模块一 session"约束
——lhx 首次在真实 Haiku 上清晰正向 delta 的任务，见 §5.9）**，均带 `verify` 可执行检查，§5.9）。
每个任务带参考解（证明可解）；schema 见
...
```bash
lhx-eval run --backend sdk --verified-only -k 1   # 6 trial, $0.33
```
...
## 7. 局限与未来工作
- **跨会话协调 A/B 已捕获（v05）；跨 compaction 多特性 A/B 仍是缺口。** v05 通过"prompt 显式约束
  一模块一 session + 完成 gate 强制续到 verify 通过"路径已实现清晰的 ON>OFF（§5.9）。**剩下的**：
  把 v05 的多特性结构和 §5.9 的 prompt 内联 168k tokens compaction 触发合并，测 M3（context rot 后
  ledger 是否救得回来）。再加**注入真实中断**（杀 `claude -p` 子进程 fresh 重启）直接考 M7 的
  checkpoint+resume——SDK 路径可以直接 `raise CancelledError`，比 CLI SIGKILL 干净。
...
- **v05 k=3 已给出显著信号；k≥5 会把 McNemar 从 p=0.25 推到 p<0.05**（当前 k=3 上限为 exact-test
  的样本量限制，非效应量问题）。同法可在 Haiku/Sonnet 对照跑，画真实 pass^k / **成本·会话数** 曲线。
