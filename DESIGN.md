...
（§2）。但本方案的主体是**如何严肃地评估这层策略**：一套配对 A/B harness，解耦
模型 / Harness / Tasks / Eval（§5.10），支持两种 backend（确定性仿真 / 真实 Claude）、多种
grader（可执行 F2P / token / model-judge / 以及被明确拒绝的"自报"反模式）。

方法学分两层，对应两类**性质不同**的数字——刻意不把它们混为一谈：

1. **验证评估器本身（instrument validation，不是 evaluation）。** 先用一个植入了**已知效应**
   的仿真把尺子校准：确认 harness 在该报阳性时报阳性、在阴性对照（regression 子集）上不误报、
   统计与沙箱都正确（§5.8）。*仿真 agent 不是业界意义上的评估方法*（业界跑真实 agent，如
   Terminal-Bench/SWE-bench/OSWorld），所以这些数字只作"尺子准不准"的证据、**不作为成绩**。
2. **真实 evaluation（business 上真正关心的）。** 在真实 Claude（Haiku）上用**可执行验证**
   （F2P，非自报）跑 A/B（§5.9）。**诚实结论：** 在当前任务规模上，模块对成功率是**中性的**、
   在不需要长程支持时甚至是**净开销**（多会话、更高成本）；评估**正确地捕捉到了这一点**。要
   看到长程增益，必须有真正**超出单会话**的任务——最务实的路径是借 Harbor adapter 接入
   Terminal-Bench 等现成长程套件（§5.10 / §7）。

换言之，本方案最有价值的不是某个漂亮数字，而是一套**能分清"尺子准不准"与"模块好不好"、并能
诚实报出模块何时有用 / 何时添乱**的评估系统。具体的仿真校准数字见 §5.8，真实 A/B 见 §5.9。
...
### 5.3 任务集
当前 13 个合成任务（[scripts/seed_tasks.py](scripts/seed_tasks.py) 生成并校验）：**capability**
子集（多文件构建、refactor/迁移链、CLI、ETL、bug 扫除，跨 compaction 边界且/或被中断）；
**regression** 子集（短、干净，两臂≈100%，抓回退）；以及 **executable-verified** 任务
（`v01-slugify` / `v02-health-endpoint`（regression）、`v03-tasklib`（capability，4 模块+pytest）、
`v04-bigrepo-audit`（capability，含 `setup` 种入 50 模块的大仓库以施加 context 压力），均带
`verify` 可执行检查，§5.9）。每个任务带参考解（证明可解）；schema 见
...
> 这两栏自然是 "—"——这是"分母为零"而非"模块失效"。要让它们出数，必须先制造真实的 compaction /
...
在相信任何数字前做的事——这是评估工程最该被考核的部分。**注意：本节的仿真数字是"尺子校准"，
不是模块的能力成绩**（仿真 agent 不是业界评估方法）。
...
- `v04-bigrepo-audit`（capability）：`setup` 种入 **50 个臃肿模块（约 178k tokens）**，要求给每个加
  `AUDITED=True`，意在**逼出真实 compaction**。
...
**试图逼出真实 compaction（v04），以及一个关于 agent 的反直觉发现。** 为了在真实 Haiku 上触发
context warning，v04 用 `setup` 种入约 178k tokens 的大仓库。实跑结果出人意料：Haiku **只用 11
个工具调用、$0.079** 就给全部 50 个模块加上了 `AUDITED`、`success=True`，**完全没有 compaction**
（`crossed_compaction=False`）。原因是它**用 shell 批处理（sed/循环）绕过了逐文件阅读**，那 178k
tokens 从未进入它的 context。教训：**内容在磁盘上能被 grep 绕开，必须把它塞进 context 才算数。**

**于是把内容直接拼进 prompt——成功逼出了真实 compaction。**
[scripts/context_pressure_probe.py](scripts/context_pressure_probe.py) 把 50 个模块**内联进 prompt**
（≈168k tokens），内容从第 1 个 turn 就在 context 里、无法被 grep 绕开。实跑：`events={'compaction':
1, 'tool_use': 19}`——**PreCompact hook 记录到一次真实的 Haiku 自动 compaction**（$0.249）。这确认了
长程 regime 在真实 Haiku 上**可按需触发**：诀窍是 (a) 内容必须**在 context 内**（prompt 内联，而非
磁盘），且 (b) 量级要够（~168k；42k 不触发）。注意：单条 prompt 不能超过窗口（否则是硬报错而非
compaction），故真正的 compaction 来自**多 turn 对话增长**越过阈值。**诚实边界：** 触发 compaction
证明了*机制*；要证明*模块有用*，还需一个"跨 compaction 的多特性任务"（compaction 后 OFF 丢轨、ON
靠 completion gate + ledger 续完）——这是下一步（§7）。无 context 上限 CLI flag 时，这条 prompt-内联
路线 + §5.9 的收紧预算 multi-session 路线，是触发长程压力的两个可靠手段。

**多会话真实实验（M4/M7 的真正考验）。** 单次 `claude -p` 只压 M1/M3；模块的核心价值在
**跨会话**。为此加了 multi-session 模式（`max_sessions` / `LHX_SDK_MAX_SESSIONS`）：在同一持久
工作区里**反复 fresh 跑 `claude -p`**（不 `--continue`，每会话用 `--max-turns` 限死，谁都无法
一次做完），会话间用可执行检查判收敛。在 v03 上实跑（7 turns/会话，上限 5 会话）：

| arm | success | sessions | hook 事件 | cost |
|---|---|---|---|---|
| ON | True | 3 | 32 | $0.111 |
| OFF | True | **1** | 0 | **$0.048** |

**这是个诚实且有信息量的结果（而非粉饰的胜利）：** OFF 一个 7-turn 会话就做完了——说明 v03
对 Haiku 仍是"单会话量级"任务；此时 ON 的 completion gate 强制逐特性验证 + 每会话被截断，
反而**更慢更贵**（3 会话、2.3× 成本，成功率却一样）。换言之，在不需要长程支持的任务上，模块
是**净开销**——而评估正确地**捕捉到了**这一点（对应 JD 里的"Tokens 效率/资源利用率"维度，也
印证 Anthropic"harness 组件编码假设、不需要时应移除"的原则）。它精确界定了适用边界：只有当
任务**真的超出单会话**（即便最简实现也做不完）时，长程机制才可能由负转正。

> 这就是"评估真实表现"的正确形态（Anthropic 路线）：跑真实 agent、用**可执行测试**判真实
> 产出，并**同时计成本/会话数**——这样既能看到模块帮忙，也能看到它在不该用的地方添乱。仿真层
> 只是先确保尺子准的脚手架。要在真实 Claude 上看到 ON<OFF，需要比 v03 大得多、单会话也做不完
> 的可验证长程任务——而这些任务**不必自己造**（见 §5.10）。
...
Docker 镜像与大量 credit-hours，单 Terminal-Bench 一轮 ON/OFF×k 即可观。但鉴于解耦同构、且
`verify` 与其评分等价，所需的是一个 adapter 而非重新设计；本方案的离线仿真 + 小规模真实 A/B
已验证了这条链路的每一环。基线之难也佐证了长程远未饱和：APEX-Agents 顶级模型 Pass@1 仅约
24%、Terminal-Bench <65%。
...
## 7. 局限与未来工作
- **跨 compaction 的多特性 A/B——这是当前最大的缺口（但路已探通）。** 触发真实 compaction 已
  解决（§5.9：prompt 内联 ~168k tokens → Haiku 真 compaction）；收紧每会话预算的 multi-session
  装置也已就绪。**剩下的一步**是把两者合成一个任务：一个 prompt 内联到迫近窗口、含多个
  executable-verified 特性的任务，使 compaction 在中途发生——预期 OFF 在 compaction 后丢轨/提前
  收手，ON 靠 completion gate + on-disk ledger 续完。再加**注入真实中断**（杀进程 fresh 重启）直接
  考 M7。这才是能让 ON>OFF 的真实长程 A/B。
...
- 在 Haiku/Sonnet 上把上面的设定跑到 k≥5，得到真实的 pass^k / **成本·会话数** 曲线。
