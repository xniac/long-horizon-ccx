...
**局限**：现在的 agent 很强、会自动调工具，多数任务一个 session 就做对了，ON 反而只多付通信开销（§5 的 v01–v03 行）。OFF 也能成功，是因为总有逃生口：pytest 给确切 N/M、文件系统暴露历史决策、任务塞得进单会话、或 shell 批处理绕过 context——这是 v03/v04 与一系列未收录的探索任务（cascading-bug、config-driven、orphan-file 等变体，最终没进任务集）一路跑出来的教训。
...
doom-loop 频次），以及**把 token/资源效率当一等公民**（v07 唯一的收益就是效率，§5.3）。
...
**同 v06 的保留也适用于此**：prompt 要求「一 session 只建一个模块」而 max_sessions(3) 远小于模块数(10)，遵守指令的 OFF 在算术上就不可能完成；ON 赢在门控迫使它突破这条指令、在一个 session 里连做多个模块。所以这个 delta 同样带构造成分——它是门控机制的干净演示，不是野外发现（§6 列了去构造化的跑法）。
...
### 5.4 评估过程中抓出的两个脚手架 bug（拿到 v05 正 delta 之前）
...
**不受**沙箱目录约束（它能写本用户能写的任何路径）。这是刻意的取舍——按本仓库的任务集风险可控，
